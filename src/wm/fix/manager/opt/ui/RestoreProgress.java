...
import wm.fix.manager.opt.install.RestoreTask;
...
public class RestoreProgress extends Dialog
{
    ...
    private RestoreTask m_task = null;
    ...
    RestoreProgress m_rp = null;
 
	private String m_lang = "";
	private FixManagerUIResourceBundle m_resource = null;
    
    public RestoreProgress(Shell parent, FixProfile fp) 
    {
	...
	public int open()
	{
		...
		dialog.setText(m_resource.getString(FixManagerConstants.CNF_RESTORE_DLG_TITLE));
    ...
    public void runTask()
    {
    	RestoreTaskThread task = new RestoreTaskThread(dialog.getDisplay());  
	...
	{
		RestoreProgress m_frp = null;
		
		public startMouseListener(RestoreProgress frp)
		{
			m_frp = frp;
		}
	...
	/**
	 * RestoreTaskThread class run as separate thread to perform
	 * restore task for profile.
	 * @author xiandros
	 *
	 */
	protected class RestoreTaskThread extends Thread 
	{
	    protected Display display;

	    public RestoreTaskThread(Display display) 
	    {
	        	...
	        	if (!display.isDisposed()) 
	        	{
	                display.syncExec(new Runnable() {
	                	public void run() {	                                    	
	                			if (m_fp != null)
	                            {
	                                m_task = new RestoreTask(m_fp,m_rp);
	                                setProgressBar();
	                                m_task.go();
	                            }
	                    }
	...
	} // End of RestoreTaskThread class

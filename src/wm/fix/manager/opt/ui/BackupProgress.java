...
import wm.fix.manager.opt.install.BackupTask;
...
public class BackupProgress extends Dialog
{
    private String newline = "\n";
    private String m_message = "";
    private BackupTask m_task = null;
    ...
    BackupProgress m_bp = null;
 
	private String m_lang = "";
	private FixManagerUIResourceBundle m_resource = null;
    
    public BackupProgress(Shell parent, FixProfile fp) 
    {
	...
	public int open()
	{
		...
		dialog.setText(m_resource.getString(FixManagerConstants.CNF_BACKUP_DLG_TITLE));
    ...
    public void runTask()
    {
    	BackupTaskThread task = new BackupTaskThread(dialog.getDisplay());  
	...
	{
		BackupProgress m_fbp = null;
		
		public startMouseListener(BackupProgress fbp)
		{
			m_fbp = fbp;
		}
	...
	/**
	 * BackupTaskThread class run as separate thread to perform
	 * backup task for profile.
	 * @author xiandros
	 *
	 */
	protected class BackupTaskThread extends Thread 
	{
	    protected Display display;

	    public BackupTaskThread(Display display) 
	    {
	        	...
	        	if (!display.isDisposed()) 
	        	{
	                display.syncExec(new Runnable() {
	                	public void run() {	                                    	
	                			if (m_fp != null)
	                            {
	                                m_task = new BackupTask(m_fp,m_bp);
	                                setProgressBar();
	                                m_task.go();
	                            }
	                    }

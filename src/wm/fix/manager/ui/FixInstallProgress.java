...
import wm.fix.manager.install.InstallerTask;
import wm.fix.manager.util.FixUtil;

public class FixInstallProgress  extends Dialog
{
    private boolean m_install = true;
    private String newline = "\n";
    private String m_message = "";
    private InstallerTask m_task = null;
    FixData m_fd = null;
    ...
    FixInstallProgress m_fip = null;
	private String m_lang = "";
	private FixManagerUIResourceBundle m_resource = null;
    
    public FixInstallProgress(Shell parent, FixData fd, boolean install) 
    {
    ...
    public FixInstallProgress(Shell parent, FixProfile fp, boolean install) 
    {
    	super(parent, SWT.APPLICATION_MODAL);
    	m_install = install;
    	m_fp = fp;
...
//        	System.out.println("### "+Integer.toString(m_task.getCurrent()));
	...
	class startMouseListener implements MouseListener
	{
		...
		public startMouseListener(FixInstallProgress fip)
		{
			m_fip = fip;
		}
	        	...
	        	if (!display.isDisposed()) 
	        	{
	                display.syncExec(new Runnable() {
	                	public void run() {	                                    	
	                			if (m_fp != null)
	                            {
	                                m_task = new InstallerTask(m_fp,m_fip);
	                                setProgressBar();
	                                //m_task.go(m_install);
	                                m_task.go();
	                            }
	                            ...
	                            {
	                                m_task = new InstallerTask(m_fd,m_fip);
	                                setProgressBar();
	                                //m_task.go(m_install);
	                                m_task.go();
	                            }
